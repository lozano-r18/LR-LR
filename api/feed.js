...
export default async function handler(req, res) {
  try {
    ...
    const parser = new XMLParser({ 
      ...
      stopNodes: ['*.desc', '*.de', '*.en', '*.es', '*.fr', '*.nl', '*.pl', '*.ru', '*.da', '*.no', '*.sv', '*.cs']
    });
    ...
    const lightweight = properties.map(node => {
      try {
        ...
        const descString = typeof node.desc === 'string' ? node.desc : JSON.stringify(node.desc || '');
        const cleanDesc = descString.replace(/<[^>]+>/g, '').replace(/<!\[CDATA\[/g, '').replace(/\]\]>/g, '').trim();
        ...
        return {
          ...
          desc: { en: cleanDesc.split('. ').slice(0, 2).join('. ') + '.' }
        };
      } catch(e) { return null; }
    }).filter(Boolean);
